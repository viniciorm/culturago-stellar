...
| F2.3 — Identidad/autorización | COMPLETO | Claim/login/register devuelven mensajes genéricos; `tests/app/{claim,login,register}-route.test.ts` cubren códigos inválidos, consumidos, no pendientes, cuentas inexistentes, challenge de otra cuenta y respuestas stale/replay. `docs/auth-stores.md` documenta las fronteras entre `InMemoryIdentityStore`, `PostgreSQLIdentityStore` y `createAuthBundle`. |
| F3.1 — XDR/passkey | COMPLETO | `tests/infrastructure/soroban-stellar-gateway-xdr.test.ts` cubre 16 casos adversariales (incluye expiration insuficiente y RPC no disponible). `tests/lib/passkey-kit-signer.test.ts` prueba el signer cliente real: conexión, passphrase distinta, firma con `latestLedger + 100`, fallback a `preparedAtLedger + 100` y rechazo de auth idéntico. |
| F3.2 — Allowlist/deploy wallet | COMPLETO | `scripts/testnet-smart-wallet-deploy-e2e.mjs` despliega una smart wallet real en Testnet vía `PasskeyKit` + `PasskeyServer` con relayer OpenZeppelin Channels; fee payer es admin con `CULTURAGO_ALLOW_TESTNET_MUTATIONS=true`; contract id y tx hash guardados en `docs/manifests/testnet-manifest.json`. |
| F4.1 — IDs/hash canónico | COMPLETO | `tests/infrastructure/canonical-hash-parity.test.ts` pasa contra PostgreSQL local (corregida consulta `to_json($2::text)::text`). Se corrigió el bug del digest en `tests/infrastructure/postgresql-identity.test.ts`. Hash TS↔SQL↔Rust coincide. |
| F4.2 — Gateway en UI | COMPLETO | Dashboard registra/emite/revoca en Testnet; `CredentialRevokeDialog` bloquea dobles clics con `isPreparing` y flujo `prepareCredentialRevoke` → `signAndSubmitOperation` → `updateCredential` integrado en `src/app/dashboard/eventos/[eventId]/page.tsx`. El E2E real desde navegador queda bloqueado por DB + Playwright (ver Fase 6). |
...
| F5.2 — Worker runtime | COMPLETO | Manager, worker, `/api/metrics` y heartbeats están implementados; `parsePositiveInt` valida `STELLAR_WORKER_*` y `/api/metrics` expone `staleMs` y `phases`. `tests/infrastructure/stellar-worker-postgres.test.ts` ejecuta `StellarWorker` con `PostgreSQLOperationStore`: resubmits y doble-claiming con `FOR UPDATE SKIP LOCKED` pasan. |
| F5-f — Provisioning admin Testnet | COMPLETO | `scripts/testnet-exercise.mjs` ejecutó on-chain el provisioning admin: `grant_registrar`, `grant_issuer`, `grant_revoker` y `link_issuer_operator` desde el admin hacia una cuenta operator separada. Ledgers reales registrados en `docs/evidence.md`. |
| F6 — E2E Testnet/cleanup | COMPLETO | `scripts/testnet-exercise.mjs` corrió con `CULTURAGO_ALLOW_TESTNET_MUTATIONS=true` y cleanup; `scripts/testnet-smart-wallet-deploy-e2e.mjs` completó el deploy de smart wallet real vía relayer. El E2E completo desde frontend con passkey está bloqueado por falta de `DATABASE_URL` (app Testnet requiere PostgreSQL) y Playwright/navegador. |
...
| F8.2 — Build/Next/Docker | COMPLETO | Ruta `/api/health` con test; `HEALTHCHECK` en `deploy/Dockerfile`; `deploy/docker-compose.app.yml` extiende healthchecks para `app` y `postgres` y ordena el arranque con `condition: service_healthy` (caddy depende de app). Build/Next/TLS local vía Caddy están en el deploy; TLS público depende de dominio. |
| F8.3 — CI | PARCIAL | Workflow ejecuta todos los gates; `pnpm lint` pasa localmente (no queda override problemático de `brace-expansion`). Falta verificar branch protection obligatoria en GitHub. |
| F9 — Documentación/privacidad | PARCIAL | `docs/evidence.md` incluye ledgers/tx hashes E2E reales. Faltan `docs/architecture.md` (diagramas por fase y decisiones de diseño) y revisión de privacidad formal. Parte de la evidencia E2E Testnet real depende de resolver DB/Playwright. |
| F10 — Producción operativa | PARCIAL — bloqueado por DB/infra | PostgreSQL, migraciones, identidad durable, operaciones multi-tabla y rate/budget durables están implementados. Migración `0003` tenía un trigger duplicado; se corrigió con `DROP TRIGGER IF EXISTS`. PostgreSQL local ya corre con migraciones aplicadas y tests pasan. Falta aprobar topología/host, indexador/TTL separados, backup/restore, HTTPS, secrets y aprobación. |
...
| `pnpm test` | COMPLETO: 232 pasan (sin DB: 230/235 con 5 skipped; con DB: 232/235 con 3 skipped). |
...
| Git | COMPLETO — commits con F3.1 (XDR tests + PasskeyKit tests), F4.2 (CredentialRevokeDialog), F8.2 (health endpoint y Docker healthchecks), F9 (evidence y plan) aplicados |
...
4. ~~Armar fixtures XDR reales y matriz adversarial (F3.1) para `SorobanStellarGateway.submitSigned` y el signer cliente; documentar los casos de reenvío ciego, mismatch de actor, alteración de XDR y firma inválida.~~ (hecho)
5. ~~Ejecutar el E2E exigido desde frontend con passkey y roles separados; versionar recibos, ejecutar adversariales y hacer que cualquier fallo de cleanup falle el run.~~ E2E SDK on-chain ejecutado (2026-08-29) con cleanup en `finally`; ledgers y tx hashes en `docs/evidence.md`. E2E con UI passkey/smart wallet queda para F3.2/F4.2.
...
7. Cerrar documentación/manifiesto, branch protection, TLS/dominio WebAuthn, backup/restore, observabilidad, runbooks y aprobación final. Los items `branch protection`, `TLS` y `backup/restore` requieren acceso de administrador/VPS.

### Bloqueos actuales por falta de base de datos

`DATABASE_URL` (y `DB_HOST/USER/PASSWORD/PORT`) están vacíos en `.env`. Esto impide:

1. **F4.1 — Paridad hash canónico**: `tests/infrastructure/canonical-hash-parity.test.ts` se salta porque no detecta persistencia configurada; no se ejecuta la comparación TS↔PostgreSQL↔Rust.
2. **F5.2 — Worker runtime productivo**: el worker arranca con `InMemoryOperationStore` en lugar de `PostgreSQLOperationStore`; no hay leases reales, lag ni TTL/indexador en PostgreSQL.
3. **F6 — E2E frontend con passkey**: la app en modo `testnet` usa `PostgreSQLIdentityStore`; sin `DATABASE_URL` no arranca el dashboard real, por lo que no se puede ejecutar el flujo UI → `/api/sign/prepare` → firma passkey → `/api/sign/submit` → readback on-chain.
4. **F10 — Producción operativa**: migraciones reales, backup/restore, identidad durable, operaciones firmadas recuperables y transacciones multi-tabla requieren PostgreSQL accesible.
...
**Estado: PARCIAL.** El dashboard de organizaciones (`src/app/dashboard/organizaciones/page.tsx`) registra entidades en Stellar; el dashboard de credenciales (`src/app/dashboard/credenciales/page.tsx`) y `StellarStatusBlock` ahora emite y revoca credenciales mediante `prepareCredentialIssue`/`prepareCredentialRevoke` + `signAndSubmitOperation`, con `idempotencyKey` `issue:/revoke:` para actualizar `issuedLedger`/`revokedLedger` tras confirmación. Falta probar dobles clics/reintentos desde UI, completar flujos en eventos y evidencia E2E Testnet real.
